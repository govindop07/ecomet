...
const CartPage = () => {
  const { cartItems, products, cartCount, currency, updateQuantity } =
    ...
    <div className="border-t pt-14 max-w-[90%] mx-auto text-2xl mb-3">
      ...
      {cartCount == 0 ? (
        <div className="text-gray-500 text-start mt-20">
          Your cart is empty!  
          <Link className="text-blue-400 text-xl" to={'/'}> Click here to add products</Link>
        </div>
      ) : (
        ""
      )}
      ...
      <div>
        {cartData.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="py-4 border-t border-b border-b-gray-200 text-gray-700 grid grid-cols-[4fr_0.5fe_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4"
            >
              <div className="flex items-start gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-5 mt-2">
                    ...
                    <p className="text-sm px-2 sm:px-3 sm:py-1 border bg-slate-50">
                      {item.size}
                    </p>
                  </div>
                </div>
